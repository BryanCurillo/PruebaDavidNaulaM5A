...
public class DepartamentoController {
    ...
    public ResponseEntity<List<Departamento>> listarDepartamento() {
        return new ResponseEntity<>(departamentoService.findByAll(), 
                HttpStatus.OK);
    }
    @PostMapping("/crear")
    public ResponseEntity<Departamento> crearDepartamento(
            @RequestBody Departamento p) {
        return new ResponseEntity<>(departamentoService.save(p), 
                HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<Departamento> actualizarProducto(@PathVariable Long id, @RequestBody Departamento p) {
    	Departamento personaENcontrada = departamentoService.findById(id);
        if (personaENcontrada == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else {
            try {
                personaENcontrada.setNombre(p.getNombre());
                personaENcontrada.setProfesor(p.getProfesor());
